    ...
    public partial class OrderEditWindow : Window
    {
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = _order.Id == 0
                        ? @"INSERT INTO orders (user_id, vehicle_id, start_date, end_date, total_price, status) VALUES (@uid, @vid, @start, @end, @price, @status)"
                        : @"UPDATE orders SET user_id=@uid, vehicle_id=@vid, start_date=@start, end_date=@end, total_price=@price WHERE id=@id";
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@status", _order.Id == 0 ? 1 : _order.Status);
